...
if [ -f "$APK" ]; then
    ...
    echo "  1) Install directly from Termux (recommended):"
    echo ""
    echo "     termux-open \"$APK\""
    echo ""
    echo "     Tap 'Install' in the Android installer popup."
    echo "     (If prompted, enable Unknown Sources for Termux in Settings → Security)"
    echo ""
    echo "  2) Open the APK file in the Files / Downloads app"
    echo "     (enable Unknown Sources in Settings → Security first)"
    echo ""
    echo "  3) If ADB is available (USB/wireless debug from another machine):"
    echo ""
